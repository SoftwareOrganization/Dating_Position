...
public class MyMessageReceiver extends BroadcastReceiver {
	...
	public static String INVITEJOINDATE = "invite_join_date";
	@Override
			   ...
			   //增加消息接收方的ObjectId--目的是解决多账户登陆同一设备时，无法接收到非当前登陆用户的消息。
				...
				if(fromId!=null && !BmobDB.create(context,toId).isBlackUser(fromId)){//该消息发送方不为黑名单用户
					if(TextUtils.isEmpty(tag)){//不携带tag标签--此可接收陌生人的消息
						...
						}else if (tag.equals(INVITEJOINDATE)) {
							
							if(currentUser != null){
								if(toId.equals(currentUser.getObjectId())){
									MainActivity.joinDate(jo.getString("fId"));
									// Toast.makeText(CustomApplcation.getInstance().getApplicationContext(), jo.getJSONObject("aps").getString("alert"), Toast.LENGTH_LONG).show();
									Log.i("聚会邀请消息", jo.getJSONObject("aps").getString("alert") + "消息");
									...
									if(isAllow && currentUser!=null && currentUser.getObjectId().equals(toId)){
										...
										BmobNotifyManager.getInstance(context).showNotify(isAllowVoice,isAllowVibrate,R.drawable.ic_launcher, 
												jo.getJSONObject("aps").getString("alert"), BmobJsonUtil.getString(jo, BmobConstant.PUSH_KEY_TARGETUSERNAME), 
												jo.getJSONObject("aps").getString("alert"), NewFriendActivity.class);
									}
								}
							}
						}
