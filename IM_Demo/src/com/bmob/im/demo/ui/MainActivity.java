...
//		BmobChat.getInstance(this).stopPollService();
	...
	public static void joinDate(String fId) {
		BmobQuery<DatePosition> query = new BmobQuery<DatePosition>();
		query.addWhereEqualTo("creatorObjectId", fId);
		query.findObjects(CustomApplcation.getInstance().getApplicationContext(), new FindListener<DatePosition>() {
			...
			public void onSuccess(List<DatePosition> arg0) {
				...
				for (Iterator<DatePosition> iterator = arg0.iterator(); iterator.hasNext();) {
					DatePosition datePosition = (DatePosition) iterator.next();
					
					relation.add(datePosition);
					
				}
				user.setMyDates(relation);
				user.update(CustomApplcation.getInstance().getApplicationContext(), CustomApplcation.getInstance().getCurrentuser().getObjectId(), new UpdateListener() {
